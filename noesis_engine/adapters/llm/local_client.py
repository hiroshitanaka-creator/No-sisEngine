...
class LocalStructuredGenerationError(RuntimeError):
...
class LocalLLMClient(LLMPort):
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._base_url = self._settings.local_llm.base_url.rstrip("/")
        self._api_key = self._settings.local_llm.api_key
        self._timeout_sec = self._settings.local_llm.timeout_sec
        self._model_name = self._settings.local_llm.model
    ...
    def generate_structured(
        ...
        try:
            ...
            raise RuntimeError(f"Local structured generation request failed: {exc}") from exc
        ...
        try:
            ...
            raise LocalStructuredGenerationError(
                "Local model returned malformed structured output.",
                raw_text=raw_text,
            ) from exc
    ...
    def _extract_content(self, data: dict[str, Any]) -> str:
        ...
        if not choices:
            raise LocalStructuredGenerationError("Local model response did not contain any choices.")
        ...
        raise LocalStructuredGenerationError("Local model response did not contain textual message content.")
...
__all__ = ["LocalLLMClient", "LocalStructuredGenerationError"]
